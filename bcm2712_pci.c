...
/* BCM2712-specific register offsets */
#define REG_EP_CONFIG_DATA_2712		        0x9004     /* moved from 0x8000 */
#define REG_PCIE_HARD_DEBUG_2712		0x4304     /* moved from 0x4204 */
#define REG_PCIE_MISC_PCIE_CTRL         0x4064     
#define PCIE_MISC_PCIE_CTRL_PERSTB		(1 << 2)	   /* hit 2, active low */
#define REG_PCIE_RC_PL_PHY_CTL_15               0x4234
#define PHY_CTL_15_PM_CLK_PERIOD_MASK  		0xff
#define REG_PCIE_MISC_MISC_CTRL         	0x4008
#define REG_PCIE_MISC_UBUS_CTRL        		0x4044
#define UBUS_PCIE_REPLY_ERR_DIS        		(1 << 7)
#define UBUS_PCIE_REPLY_DECERR_DIS     		(1 << 6)
#define REG_PCIE_MISC_AXI_READ_ERROR   		0x4094
#define REG_PCIE_UBUS_TIMEOUT         		0x404c
#define REG_PCIE_RC_CONFIG_RETRY_TIMEOUT 	0x405c
...
bcm_pcib_probe(device_t dev)
{
	...
	struct bcm_pcib_softc *sc = device_get_softc(dev);
	sc->is_bcm2712 = ofw_bus_is_compatible(dev, "brcm,bcm2712-pcie");

	device_set_desc(dev,
	    "BCM2712-compatible PCI-express controller");
...
bcm_pcib_perst_set_2712(struct bcm_pcibsoftc *sc, bool assert)
{
	...
	tmp = bcm_pcib_read_reg(sc, REG_PCIE_MISC_PCIE_CTRL);
	if (assert)
		tmp &= ~PCIE_MISC_PCIE_CTRL_PERSTB; /* active low */
	else 
		tmp |= PCIE_MISC_PCIE_CTRL_PERSTB;
	bcm_pcib_set_reg(sc, REG_PCIE_MISC_PCIE_CTRL, tmp);
}	
...
bcm_pcib_attach(device_t dev)
{
	...
	/* BCM2712-specific intialization */ 
	if (sc->is_bcm2712) {
		uint32_t tmp;

		/* Assert then dessert PERST# */
		bcm_pcib_perst_set_2712(sc,true);
		DELAY(100);
		bcm_pcib_perst_set_2712(sc,false);
		DELAY(100);

		/* Fix for L1SS errata - PM clock period 18.52ns */
		tmp = bcm_pcib_read_reg(sc, REG_PCIE_RC_PL_PHY_CTL_15);
		tmp &= ~PHY_CTL_15_PM_CLK_PERIOD_MASK;
		tmp |= 0x12;
		bcm_pcib_set_reg(sc, REG_PCIE_RC_PL_PHY_CTL_15, tmp);

		/* Suppress AXI error responses */
		tmp = bcm_pcib_read_reg(sc, REG_PCIE_MISC_UBUS_CTRL);
		tmp |= UBUS_PCIE_REPLY_ERR_DIS;
		tmp |= UBUS_PCIE_REPLY_DECERR_DIS;
		bcm_pcib_set_reg(sc, REG_PCIE_MISC_UBUS_CTRL, tmp);
		bcm_pcib_set_reg(sc, REG_PCIE_MISC_AXI_READ_ERROR, 0xffffffff);

		/* Timeouts - 250ms and 240ms in 750MHz clocks */
		bcm_pcib_set_reg(sc, REG_PCIE_UBUS_TIMEOUT, 0xB2D0000);
		bcm_pcib_set_reg(sc, REG_PCIE_RC_CONFIG_RETRY_TIMEOUT, 0xABA0000);
